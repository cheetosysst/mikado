...
const Layout = ({ children }: { children?: ReactNode }) => (
	<div className="bg-animation flex h-screen gap-4 px-4 md:px-0">
		<div className="my-4 flex w-14 grow-0 flex-col items-end gap-4 lg:grow">
			<Link href={`#`}>
			...
			<div className="mr-4 flex flex-col items-start gap-2">
		...
		<GlassBase className="my-4 max-h-screen w-3/5 grow overflow-hidden md:grow-0 lg:w-1/3">
			{children}
		</GlassBase>
		<div className="mt-4 hidden w-14 grow flex-col gap-4 md:flex">
	...
	return (
		<div className="flex flex-col">
			{mockTrends.map((item, index) => (
				<>
					<Link key={`trend-${index}`} href={item.url}>
